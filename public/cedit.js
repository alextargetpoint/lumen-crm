...
`;
  ...
  function slideHtml(sl) {
    ...
    ${(() => { let r = {}; try { r = JSON.parse(sl.dataset.rich || '{}'); } catch (e) {} const NUM = ['payplan', 'stats', 'steps', 'bars', 'gauges', 'growth', 'timeline']; if (!(r.mode && NUM.includes(r.mode) && (r.items || []).length)) return ''; const FMT = [['payplan', 'Рассрочка'], ['timeline', 'Сроки'], ['growth', 'Рост'], ['stats', 'Числа'], ['bars', 'Столбцы'], ['gauges', 'Кольца'], ['steps', 'Шаги']]; return `<div class="cgrp"><label>Формат блока <span style="color:var(--cb);font-weight:700">· ${(r.items || []).length} пункта</span></label><div class="cseg" id="cBlockFmt" style="flex-wrap:wrap">${FMT.map(([v, n]) => `<button data-bf="${v}" class="${r.mode === v ? 'on' : ''}" style="flex:0 0 auto">${n}</button>`).join('')}</div><div class="cnote">Одни данные — разные виды подачи. Переключай, пока не ляжет красиво.</div></div>`; })()}
