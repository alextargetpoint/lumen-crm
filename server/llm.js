...
async function extractProperty(context, url) {
  const prompt = `Ты извлекаешь МАКСИМУМ данных о ПРОЕКТЕ недвижимости со страницы листинга. Верни СТРОГО JSON:
{"name":"название","developer":"застройщик","area":"район/локация","city":"город","priceFrom":число,"currency":"USD|EUR|AED|THB","beds":"спальни","size":"площадь напр. 45 m²","type":"Studio|1BR|2BR|Villa","handover":"срок сдачи","roi":"доходность % (rental yield) если есть","appreciation":"прирост стоимости % если есть","description":"2-4 предложения о проекте","amenities":["удобства"],"paymentPlan":[{"pct":"25%","label":"при бронировании/подписании/по готовности — этап"}],"units":[{"type":"Studio|1BR|2BR","size":"кв.м","floor":"этаж","price":число,"view":"вид"}],"images":["URL фото объекта/рендеры"],"floorplans":["URL картинок ПЛАНИРОВОК (floor plan/layout, схематичные чертежи квартир)"]}
Правила: бери ТОЛЬКО что есть на странице, не выдумывай; чего нет — "" или []. paymentPlan — этапы оплаты по строкам (booking/SPA/construction/handover). units — если на странице есть таблица/список доступных юнитов. floorplans — ссылки именно на ЧЕРТЕЖИ планировок (не фото интерьеров/рендеры экстерьера). images — фото/рендеры объекта, НЕ логотипы. priceFrom — минимальная цена числом.
URL: ${url}
СТРАНИЦА:
${String(context).slice(0, 13000)}`;
  const j = await callGemini(prompt, 25000, 3000);
...
async function extractCatalog(markdown) {
  const prompt = `Со страницы КАТАЛОГА недвижимости извлеки СПИСОК всех проектов. Верни СТРОГО JSON:
{"projects":[{"name":"название проекта","area":"район/локация","priceFrom":число_или_0,"currency":"USD|EUR|THB|AED","type":"тип если есть","url":"абсолютная ссылка на страницу проекта из markdown"}]}
Правила: один проект = один элемент. name и url обязательны (url бери из markdown-ссылки рядом с проектом). Чего нет — "" или 0. НЕ выдумывай проекты. Максимум 60.
КАТАЛОГ:
${String(markdown).slice(0, 14000)}`;
  const j = await callGemini(prompt, 30000, 4000);
  if (!j || !Array.isArray(j.projects)) throw new Error('bad catalog extract');
  return j.projects;
}
...
${String(text).slice(0, 14000)}`;
  const j = await callGemini(prompt, 45000, 8000);
...
/* СТИЛЬ БРОКЕРА (few-shot «дообучение под брокера»): примеры его собственных отправленных/утверждённых
   первых касаний → ИИ подражает его тону/длине/структуре. Не копирует дословно и не тащит чужие факты. */
function brokerStyleBlock(samples) {
  const s = (samples || []).map(x => String(x || '').trim()).filter(x => x.length > 12).slice(-3);
  if (!s.length) return '';
  return '\nСТИЛЬ ИМЕННО ЭТОГО БРОКЕРА — вот как ОН сам пишет первые касания. Подражай его ТОНУ, длине, ритму, манере (эмодзи/без, «вы/ты», как здоровается, как задаёт вопрос). НО не копируй дословно и НЕ переноси факты/проекты из этих примеров в новый текст — они про других лидов:\n' + s.map((x, i) => `— пример ${i + 1}: «${x.slice(0, 600)}»`).join('\n') + '\n';
}

/* ИИ первое касание: разбор лида + готовое персональное сообщение (+ короткий анализ) */
async function composeFirstTouch(db, lead, draft, agencyName, styleSamples) {
...
6. ФОРМАТ (важно для WhatsApp): раздели сообщение на 2-3 коротких абзаца, между ними — ПУСТАЯ СТРОКА (перенос \\n\\n). Обычно так: (а) приветствие + признание выбора — первый абзац; (б) ценность проекта — второй абзац; (в) вопрос — третий абзац. НЕ пиши сплошной стеной текста в одну строку — сообщение должно читаться структурно, «дышать».
...
${adTranscript ? 'ЧТО КЛИЕНТ УСЛЫШАЛ В ВИДЕО (транскрипт озвучки креатива — опирайся на реальные факты/акценты, которые его зацепили; НЕ пересказывай дословно, вплетай естественно):\n«' + adTranscript + '»' : ''}
${adNotes ? 'ДОП. ИНФО ПО ПРОЕКТУ (менеджер добавил вручную — не озвучено в видео, но важно; используй, если релевантно):\n' + adNotes : ''}
${mentalityBlock(lead)}
${LIVE_STYLE}
${brokerStyleBlock(styleSamples)}
ДАННЫЕ ЛИДА:
${ctx}
...
Верни строго JSON:
{"message":"главный вариант первого сообщения (обращение по имени, если есть; язык = ${LANG}; без плейсхолдеров в фигурных скобках; РАЗБИТ на 2-3 абзаца реальными переносами строк между смысловыми блоками)",
...
async function composeChainStep(db, lead, step, agencyName, position, styleSamples, langOverride) {
  ...
  const prompt = `Ты — сильнейший брокер зарубежной недвижимости в агентстве «${String(agencyName || 'агентство').slice(0, 80)}». Напиши НАСТОЯЩЕЕ, готовое к отправке сообщение-касание в WhatsApp — такое, какое сильный брокер реально отправил бы ЭТОМУ клиенту по проекту из рекламы. Не сухой шаблон и не формальность: живой персональный текст с отсылкой к проекту, по которому пришёл лид.
...
- ГЛАВНОЕ: опирайся на ПОНИМАНИЕ КРЕАТИВА (ниже) — что реально в объявлении/видео, по которому пришёл лид. Вплетай факты естественно, не списком, ничего не выдумывай. Текст должен читаться как персональное касание по ЭТОМУ проекту, а не общая рассылка.
- Обращайся по имени через {name} (подставится автоматически). Другие переменные — {creative} (название проекта), {district}, {budget}, {timeline}, {type} — только там, где без них никак; в остальном пиши живой прозой по фактам ниже. Не выдумывай конкретные цифры бюджета/сроков — если надо, ставь переменную.
${adPoints.length ? 'СИЛЬНЫЕ СТОРОНЫ ПРОЕКТА (из дерева креативов, вплети 2-3 самые релевантные):\n' + adPoints.slice(0, 5).map(x => '- ' + String(x).slice(0, 160)).join('\n') : ''}
${adTranscript ? 'ЧТО КЛИЕНТ УСЛЫШАЛ В ВИДЕО (транскрипт озвучки креатива — реальные акценты, которые его зацепили; вплетай естественно, НЕ пересказывай дословно):\n«' + adTranscript + '»' : ''}
${adNotes ? 'ДОП. ИНФО ПО ПРОЕКТУ (менеджер добавил вручную):\n' + adNotes : ''}
${!adPoints.length && !adTranscript ? 'ПОНИМАНИЯ КРЕАТИВА пока нет (видео не транскрибировано) — опирайся на название проекта «' + (adName || geoName) + '» и район, без выдуманных цифр.' : ''}
${LIVE_STYLE}
${brokerStyleBlock(styleSamples)}
ЛИД, ДЛЯ КОТОРОГО ПИШЕМ:
имя: ${lead.name || '—'}
направление: ${geoName}
${adName ? 'объявление/проект, по которому оставил заявку: «' + adName + '»' : ''}
...
Верни строго JSON:
{"message":"главный вариант касания (обращение через {name}; живой текст с отсылкой к проекту; абзацы реальными переносами строк)",
 "variantB":"альтернативный вариант с другим заходом/крючком/вопросом — тоже сильный и естественный",
 "hook":"на чём построено — 3-6 слов"}`;
  const out = await callGemini(prompt, 20000, 1100);
  if (!out || typeof out.message !== 'string' || !out.message.trim()) throw new Error('bad chain-step');
  const clean = s => humanize(String(s || '').trim()).slice(0, 900);
  return { message: clean(out.message), variantB: clean(out.variantB), hook: String(out.hook || '').slice(0, 80) };
}
...
module.exports = { available, reply, summarize, transcribe, validateReply, rewrite, tidyNote, extractProperty, extractUnits, composeDeck, humanize, mentalityBlock, screenCandidate, composeCollection, composeAgencyAbout, composeFirstTouch, composeChainStep, composePostCall, composeCarousel, classifyPhotos, highlightHeadings, composeLeadPsych, composeScripts, huntIdeas, composePost, extractLaunch, parseTask, reviewCall, CAROUSEL_TEMPLATES, CAROUSEL_ANGLES, SHOOT_FORMATS, REELS_FORMULAS, generateImage, structureVisionSticker, masterStickerPrompt, MB_TEXT_MODES, pickPersona, HEROES, hasImage: () => !!OKEY, MODEL,
