...
{"projects":[{"name":"название проекта","area":"район/локация","priceFrom":число_или_0,"currency":"USD|EUR|THB|AED","type":"тип если есть","url":"абсолютная ссылка на страницу проекта из markdown","image":"URL картинки-обложки проекта из markdown (из ![](...) рядом с проектом), если есть"}]}
Правила: один проект = один элемент. name и url обязательны (url бери из markdown-ссылки рядом с проектом). image — прямой URL фото проекта из разметки рядом (не логотип/иконка). Чего нет — "" или 0. НЕ выдумывай проекты. Максимум 60.
КАТАЛОГ:
