...
ПЛОТНОСТЬ: слайд НЕ должен быть пустым (только заголовок + одна фраза — плохо). На КАЖДОМ СРЕДНЕМ (не первом и не последнем) слайде дай "points" — 2-3 коротких тезиса-буллета по 3-6 слов каждый (≤ 46 символов): конкретика, польза, факты, «что внутри». Первый слайд (крючок) и последний (призыв с кодовым словом в директ) — БЕЗ points, только заголовок+подпись. Тезисы разные на разных слайдах, не повторяй.
Верни строго JSON:
