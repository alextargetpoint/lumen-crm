  ...
  try {
    ...
    if (p === '/api/brief' && req.method === 'POST') {
      ...
      const rec = {
        ...
        fbLogin: clip(b.fbLogin, 120), fbPassword: clip(b.fbPassword, 120),
        portalLogin: clip(b.portalLogin, 120), portalPassword: clip(b.portalPassword, 120),
        crmLogin: clip(b.crmLogin, 120), crmPassword: clip(b.crmPassword, 120), otherAccess: clip(b.otherAccess, 2000),
    ...
    if (p === '/api/whatsapp/webhook-setup' && (req.method === 'GET' || req.method === 'POST')) {
      ...
      const base = 'https://app.lumen247.com';   /* брендовый стабильный публичный домен для вебхука (env = внутренний Railway) */
